...
describe("CostSparkline", () => {
  it("renders a path connecting every turn with a known cost", () => {
    const turns = [
      ...
      makeTurn({ index: 1, usage: { ...makeTurn().usage, costUsd: { known: true, value: 0.05 } } }),
      makeTurn({ index: 2, usage: { ...makeTurn().usage, costUsd: { known: true, value: 0.02 } } }),
    ];
  ...
  it("shows a fallback instead of a path when fewer than two turns have a known cost", () => {
    const turns = [
      makeTurn({ index: 0, usage: { ...makeTurn().usage, costUsd: { known: true, value: 0.01 } } }),
      makeTurn({ index: 1, usage: { ...makeTurn().usage, costUsd: { known: false, reason: "no data" } } }),
    ];
